...
class BetheBlochParametrisation:
    ...
    def add_nsigma_column(self, dataset: pl.DataFrame, **kwargs) -> pl.DataFrame:
        ...
        if kwargs.get('convert_to_beta', False):
            ...
            dataset = dataset.with_columns((pl.col('fPAbs')/(pl.col('fPAbs')**2 + ParticleMasses[self.fitted_particle]**2).sqrt()).alias(x_variable))
    ...
    def draw_expected_values(self, dataset) -> None:
